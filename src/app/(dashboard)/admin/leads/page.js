    ...
    <div className="fade-in">
      <header className="mb-6 flex justify-between items-end">
        <div>
          ...
          <p className="text-muted">
            Review, call landlords, and verify submitted properties.
          </p>
        </div>
      </header>
      ...
      <div className="grid gap-4">
        {leads.map((lead) => (
          <div
            key={lead.id}
            className="card p-6 border flex flex-col md:flex-row gap-6 items-start justify-between hover:border-gray-300 transition-colors"
          >
            <div className="flex-1">
              ...
              <div className="text-sm text-gray-700 space-y-2 mb-4">
                <p className="flex items-center gap-2">
                  ...
                  {lead.propertyAddress}, {lead.propertyArea}
                </p>
                <p className="flex items-center gap-2">
                ...
                {lead.notes && (
                  <div className="bg-yellow-50 p-3 rounded border border-yellow-100 mt-3 text-yellow-800">
                    <span className="font-bold text-xs uppercase mb-1 block">
                      Scout Notes:
                    </span>
                    {lead.notes}
                  </div>
              ...
              <p className="text-xs text-muted">
                Submitted by Scout ID: {lead.scoutId} on{" "}
            ...
            <div className="flex flex-col gap-2 w-full md:w-auto mt-4 md:mt-0">
              {lead.status === "SUBMITTED" && (
                <button
                  onClick={() => updateStatus(lead.id, "REVIEWING")}
              ...
              {(lead.status === "SUBMITTED" || lead.status === "REVIEWING") && (
                <>
                  <button
                    ...
                    className="btn btn-primary bg-green-600 hover:bg-green-700 border-none flex justify-center items-center gap-2"
                  >
                    <CheckCircle size={16} /> Approve & List
                  </button>
                  <button
                    onClick={() => updateStatus(lead.id, "REJECTED")}
                    className="btn btn-outline text-danger border-danger hover:bg-red-50 flex justify-center items-center gap-2"
                  >
